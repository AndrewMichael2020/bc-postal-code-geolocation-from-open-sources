...
Leadership can adjust labour cost, vehicle cost, and in-home visit duration globally or for one selected provider base. A target visit-share scenario can set any base from `0%` (no modeled home-care visits) through `100%` (the sole provider base for all mapped visits). An expandable all-base editor shows current and proposed shares together; manually edited bases stay fixed while untouched bases absorb the remaining percentage. The demo compares the preview with the travel-efficient plan rather than calling a more costly scenario optimized.
